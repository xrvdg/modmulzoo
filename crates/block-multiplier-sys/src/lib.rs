...
#[inline]
/// A block multiplier with 3 concurrent multiplications has 2.2 times the throughput on
/// Raspberry Pi.
...
/// A block multiplier with 4 concurrent multiplications has 2.2 times the throughput on
/// Raspberry Pi.
#[inline]
...
mod test {
    ...
    #[quickcheck]
    fn montgomery_interleaved_3(a: U256b64, b: U256b64, c: U256b64, d: U256b64) {
        ...
        let (scalar, vector) = super::montgomery_interleaved_3(&rtz, a.0, b.0, ac, bd);
        assert_eq!(domb::parallel_sub_simd_r256_no_trans(&rtz, ac, bd), vector);
        assert_eq!(yuval::parallel_reduce(a.0, b.0), scalar);
    }

    #[quickcheck]
    fn montgomery_interleaved_4(a: U256b64, b: U256b64, c: U256b64, d: U256b64) {
        let ac = zip_with(a.0, c.0, |fst, snd| Simd::from_array([fst, snd]));
        let bd = zip_with(b.0, d.0, |fst, snd| Simd::from_array([fst, snd]));

        let rtz = RTZ::set().unwrap();
        let (scalar, scalar2, vector) =
            super::montgomery_interleaved_4(&rtz, a.0, b.0, c.0, d.0, ac, bd);
        assert_eq!(domb::parallel_sub_simd_r256_no_trans(&rtz, ac, bd), vector);
        assert_eq!(yuval::parallel_reduce(a.0, b.0), scalar);
